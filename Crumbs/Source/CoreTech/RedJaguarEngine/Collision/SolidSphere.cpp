/**
 * 0===========================================================================0
 * | SolidSphere.cpp                                                           |
 * |  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  |
 * |   Author        : Andrew Gleeson                                          |
 * |   E-mail        : atgleeson@gmail.com                                     |
 * |  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  |
 * |   Date Created  : 10:32 am (Eastern) on 06/15/2010                        |
 * |   Date Edited   : 02:20 pm (Eastern) on 07/08/2010                        |
 * 0=====0===============================================================0=====0
*/

// 0===========================================================================0
// | ~ Includes ~                                                              |
// 0===========================================================================0
#include "SolidSphere.h"
#include "../Renderer/RJ_Renderer.h"

CSolidSphere::CSolidSphere(void) : IVolumeObject()
{
	m_fRadius = 0;
	m_vNormal = D3DXVECTOR3();
	IObject::SetObjectType(RJ_Config::RJ_SOLIDSPHEREVOLUME);
}
CSolidSphere::CSolidSphere(char* szName, D3DXVECTOR3 vPosition, float fRadius, float fSpringValue,
						   MATERIAL_TYPE eMatType, unsigned int uiID, bool bActive)
						   : IVolumeObject(szName, uiID, vPosition, eMatType, RJ_Config::RJ_SOLIDSPHEREVOLUME, fSpringValue)
{
	m_fRadius = fRadius;
	m_vNormal = D3DXVECTOR3();

	D3DXMATRIX matrix;
	D3DXMatrixIdentity(&matrix);
	matrix._41 = vPosition.x;
	matrix._42 = vPosition.y;
	matrix._43 = vPosition.z;
	SetWorldMat(matrix);

	if ( strcmp(szName, "goal") == 0 )
		SetObjectType(RJ_Config::RJ_GOAL);
}

float CSolidSphere::GetClosestRadius(D3DXVECTOR3 p)
{
	return m_fRadius;
}
float CSolidSphere::GetRadius()
{
	return m_fRadius;
}

bool CSolidSphere::TestAgainstSphere(D3DXVECTOR3 p, float r, unsigned int id)
{
	D3DXVECTOR3 length = p - GetPosition();

	if ( D3DXVec3LengthSq(&length) > (r + m_fRadius)*(r + m_fRadius) )
		return false;
	else
	{
		m_vCollisionPoint = GetClosestPoint(p);
		OnCollision();
		return true;
	}
}

void CSolidSphere::OnCollision()
{
	switch ( GetMaterialType() )
	{
	case BIT:
		{
			volatile tPlayEffect* tEFFECT = new tPlayEffect(EM_PLAYEFFECT, false, RJ_Config::RJ_COLLECTION, m_vCollisionPoint); 
			CEventManager::GetInstanceThreaded()->SendEventUpdateCrossToRender(ET_PLAYEFFECT,(tBaseEventData*)tEFFECT);
			tEFFECT = NULL;
		}
		break;
	case STONE:
		{
			volatile tPlayEffect* tEFFECT = new tPlayEffect(EM_PLAYEFFECT, false, RJ_Config::RJ_COLLISION, m_vCollisionPoint); 
			CEventManager::GetInstanceThreaded()->SendEventUpdateCrossToRender(ET_PLAYEFFECT,(tBaseEventData*)tEFFECT);
			tEFFECT = NULL;
		}
		break;
	case WALL:
		{
			volatile tPlayEffect* tEFFECT = new tPlayEffect(EM_PLAYEFFECT, false, RJ_Config::RJ_COLLISION, m_vCollisionPoint); 
			CEventManager::GetInstanceThreaded()->SendEventUpdateCrossToRender(ET_PLAYEFFECT,(tBaseEventData*)tEFFECT);
			tEFFECT = NULL;
		}
		break;
	}
}

D3DXVECTOR3 CSolidSphere::GetClosestPoint(D3DXVECTOR3 p)
{
	D3DXVECTOR3 length = p - GetPosition();
	D3DXVECTOR3 newPos = GetPosition();

	newPos.x += min(length.x, m_fRadius);
	newPos.y += min(length.y, m_fRadius);
	newPos.z += min(length.z, m_fRadius);

	return newPos;
}

void CSolidSphere::Render(double fDelta)
{		
	D3DXMATRIX matrix = GetWorldMat();
	RJ_RENDERER->DrawSphere( &matrix, &D3DXVECTOR4(0.0f, 1.0f, 0.0f, 1.0f) ); 	
}

D3DXVECTOR3 CSolidSphere::GetNormal(D3DXVECTOR3 position)
{
	D3DXVec3Normalize(&m_vNormal, &(position - IVolumeObject::GetPosition()));
	return m_vNormal;
}