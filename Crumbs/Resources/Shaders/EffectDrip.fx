...
>;
sampler2D      s2d_Diffused = sampler_state
{
   Texture = (s2d_Diffused_Tex);
   ADDRESSU = WRAP;
   ADDRESSV = WRAP;
...
float4 Crumbs_Effects_Pass_0_Pixel_Shader_mainPS( PS_INPUT Input ) : COLOR0
{
   ...
   float2 TexcoordOffset = Input.Texcoord;
     TexcoordOffset.y -= f_OffsetY * 1.7;
     
   finalColor = tex2D( s2d_Diffused, TexcoordOffset );
